...
export async function createCoupon(data) {
  try {
    ...
    if (Array.isArray(data.userIds) && data.userIds.length > 0) {
      ...
      const uniqueIds = Array.from(new Set(data.userIds.map(String)));
      const invalidIds = uniqueIds.filter(id => !id || typeof id !== 'string');
      if (invalidIds.length > 0) {
        const err = new Error(`Invalid userIds: ${invalidIds.join(', ')}`);
    ...
    return await prisma.coupon.create({ data: payload, include: { assignments: true } });
  } catch (error) {
  ...
  try {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (Array.isArray(data.userIds)) {
        // Validate userIds are non-empty strings
        const uniqueIds = Array.from(new Set(data.userIds.map(String)));
        const invalidIds = uniqueIds.filter(id => !id || typeof id !== 'string');
        if (invalidIds.length > 0) {
          const err = new Error(`Invalid userIds: ${invalidIds.join(', ')}`);
        ...
        await tx.couponAssignment.deleteMany({
          where: { couponId: Number(id), NOT: { userId: { in: uniqueIds } } }
        });
      ...
      const result = await tx.coupon.findUnique({
        where: { id: Number(id) },
        include: { assignments: true }
      });
      return result;
...
export async function deleteCoupon(id) {
  try {
    return await prisma.coupon.delete({ where: { id: Number(id) } });
  } catch (error) {
...
export async function validateCoupon({ userId, code, cartTotal, categoryIds }) {
  try {
    ...
    if (Array.isArray(coupon.assignments) && coupon.assignments.length > 0) {
      ...
      if (!userId || !assignedUserIds.includes(userId)) return { valid: false, reason: 'NOT_ASSIGNED' };
    }
