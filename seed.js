...
import { PrismaClient } from '@prisma/client';
const prisma = new PrismaClient();
// Inspect DB enum values for CouponType, add missing enum label if required,
// then convert legacy rows that used 'FLAT' to the new enum value.
async function migrateCouponType() {
	try {
		// List types that look like the coupon enum
		const types = await prisma.$queryRaw`
			SELECT t.typname
			FROM pg_type t
			WHERE t.typname ILIKE '%coupon%'
			OR t.typname ILIKE '%coupontype%'
		`;
		console.log('Possible enum types matching coupon:', types);

		// List enum labels for the expected enum type name
		...
		`;
		console.log('Existing CouponType labels:', labels.map(r => r.enumlabel));

		// If FIXED_AMOUNT is missing, add it to the enum type in Postgres.
		const hasFixed = labels.some(r => r.enumlabel === 'FIXED_AMOUNT');
		if (!hasFixed) {
			console.log('Adding FIXED_AMOUNT to Postgres enum CouponType');
			// Use executeRawUnsafe for DDL; safe here because value is static.
			await prisma.$executeRawUnsafe(`ALTER TYPE "CouponType" ADD VALUE IF NOT EXISTS 'FIXED_AMOUNT'`);
		}

		// Now update any legacy rows that used the string 'FLAT'
		const updated = await prisma.$executeRaw`
			UPDATE "coupons"
			SET "type" = ${'FIXED_AMOUNT'}::"CouponType"
			WHERE "type"::text = ${'FLAT'}
		`;
		console.log('Rows updated (legacy FLAT -> FIXED_AMOUNT):', updated);
	} catch (err) {
		console.error('Error migrating CouponType:', err);
		throw err;
	} finally {
		await prisma.$disconnect();
	}
